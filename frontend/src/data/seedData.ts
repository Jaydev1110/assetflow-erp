...
import { Asset, Department, Booking, MaintenanceTicket, NotificationLog, UserProfile } from '../types';
...
// Helper wrapper functions for LocalStorage persistence
export const loadData = <T>(key: string, initialData: T): T => {
  try {
    // Keep user sessions persistent locally for smooth demo
    if (key === 'is_logged_in' || key === 'user_profile') {
